...
function makeMap(str, expectsLowerCase) {
  ...
  for (let i = 0; i < list.length; i++) map2[list[i]] = true
  ...
  if (isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      const normalized = isString(item)
        ...
        : normalizeStyle(item)
      if (normalized) {
        for (const key in normalized) res[key] = normalized[key]
      }
    }
    return res
...
const initDepMarkers = ({ deps }) => {
  if (deps.length) {
    for (let i = 0; i < deps.length; i++) deps[i].w |= trackOpBit
  }
}
...
function trigger(target, type, key, newValue, oldValue, oldTarget) {
  ...
  if (type === 'clear') {
    ...
    depsMap.forEach((dep, key2) => {
      if (key2 === 'length' || key2 >= newValue) deps.push(dep)
    })
  } else {
  ...
  if (deps.length === 1) {
    if (deps[0]) {
      if (true) triggerEffects(deps[0], eventInfo)
    ...
    if (true) triggerEffects(createDep(effects), eventInfo)
...
function triggerEffects(dep, debuggerEventExtraInfo) {
  ...
  for (const effect2 of effects) {
    if (effect2.computed) triggerEffect(effect2, debuggerEventExtraInfo)
  }
  for (const effect2 of effects) {
    if (!effect2.computed) triggerEffect(effect2, debuggerEventExtraInfo)
  }
...
function createReactiveObject(
  ...
  if (!isObject(target)) {
    if (true) console.warn(`value cannot be made reactive: ${String(target)}`)
    ...
    registerDep(instance, setupRenderEffect) {
      ...
      instance.asyncDep
        ...
        .then((asyncSetupResult) => {
          ...
          if (true) pushWarningContext(vnode2)
          ...
          if (placeholder) remove2(placeholder)
...
function normalizeSuspenseSlot(s) {
  ...
  if (isArray(s)) {
    ...
    if (!singleChild) warn2('<Suspense> slots expect a single root node.')
...
function provide(key, value) {
  if (!currentInstance) {
    if (true) warn2('provide() can only be used inside setup().')
  } else {
  ...
  if (instance) {
    ...
    if (provides && key in provides) return provides[key]
      ...
      return treatDefaultAsFactory && isFunction(defaultValue)
        ...
        : defaultValue
    else if (true) warn2(`injection "${String(key)}" not found.`)
  } else if (true) {
...
function doWatch(
  ...
  return () => {
    effect2.stop()
    if (instance && instance.scope) remove(instance.scope.effects, effect2)
  }
}
...
const KeepAliveImpl = {
  ...
  setup(props, { slots }) {
    ...
    let current = null
    if (true) instance.__v_cache = cache
    ...
    sharedContext.activate = (vnode, container, anchor, isSVG, optimized) => {
      ...
      if (true) devtoolsComponentAdded(instance2)
    }
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      const get2 = isFunction(opt)
        ...
        : NOOP
      if (get2 === NOOP) warn2(`Computed property "${key}" has no getter.`)
...
function initProps(instance, rawProps, isStateful, isSSR = false) {
  ...
  if (true) validateProps(rawProps || {}, props, instance)

  if (isStateful) {
    instance.props = isSSR ? props : shallowReactive(props)
  } else {
    if (!instance.type.props) instance.props = attrs
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray(raw)) {
    ...
    if (!isObject(raw)) warn2('invalid props options', raw)
...
function validatePropName(key) {
  ...
  else if (true) warn2(`Invalid prop name: "${key}" is a reserved property.`)
...
function createAppAPI(render2, hydrate2) {
 ...
 If you want to mount another app on the same host container, you need to unmount the previous app by calling \`app.unmount()\` first.`)
          ...
          if (isHydrate && hydrate2) hydrate2(vnode, rootContainer)
          else render2(vnode, rootContainer, isSVG)
        ...
        if (rawRef.f) {
          ...
          if (isUnmount) {
            ...
            if (!isArray(existing)) {
              if (_isString) {
                refs[ref2] = [refValue]
                if (hasOwn(setupState, ref2)) setupState[ref2] = refs[ref2]
              } else {
                ref2.value = [refValue]
          ...
          refs[ref2] = value
          if (hasOwn(setupState, ref2)) setupState[ref2] = value
        } else if (_isRef) {
          ref2.value = value
...
- Server: ${JSON.stringify(vnode.children)}`)
    ...
    if (ref2 != null) setRef(ref2, null, parentSuspense, vnode)
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patch = (
    ...
    switch (type) {
      ...
      case Static:
        ...
        else if (true) patchStaticNode(n1, n2, container, isSVG)
      ...
      default:
        ...
        else if (true) warn2('Invalid VNode type:', type, `(${typeof type})`)
    }
  ...
  const setupRenderEffect = (
    ...
    const componentUpdateFn = () => {
      if (!instance.isMounted) {
        ...
        let vnodeHook
        if (true) pushWarningContext(next || instance.vnode)
  ...
  const unmount = (
    ...
    if (ref2 != null) setRef(ref2, null, parentSuspense, vnode, true)
...
function _createVNode(
  ...
  if (!type || type === NULL_DYNAMIC_COMPONENT) {
    if (!type) warn2(`Invalid vnode type when creating vnode: ${type}.`)
...
function getContext() {
  ...
  if (!i) warn2('useContext() called without active instance.')
...
function initCustomFormatter() {
  ...
  function genRefFlag(v) {
    if (isShallow2(v)) return 'ShallowRef'
...
function patchAttr(el, key, value, isSVG, instance) {
  if (isSVG && key.startsWith('xlink:')) {
    ...
    else el.setAttribute(key, isBoolean2 ? '' : value)
  }
}
  ...
  _resolveDef() {
    ...
    const resolve2 = (def2) => {
      ...
      for (const key of Object.keys(this)) {
        if (key[0] !== '_') this._setProp(key, this[key], true, false)
      }
    ...
    if (asyncDef) asyncDef().then(resolve2)
  ...
  _applyStyles(styles) {
    if (styles) {
      styles.forEach((css) => {
        ...
        if (true) (this._styles || (this._styles = [])).push(s)
      })
    }
  }
}
  ...
  setup(props, { slots }) {
    ...
    return () => {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        else if (true) warn2('<TransitionGroup> children must be keyed.')
      }
      if (prevChildren) {
